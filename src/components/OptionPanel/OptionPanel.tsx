...
export default function OptionPanel() {
  const { options, toggleAnswer, removeOption, mode, setMode } = useEditorStore()
          ...
          {options.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-sm">
                캔버스에서 객체를 선택한 후<br />+ 버튼을 클릭하여 선택지를 추가하세요
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="w-16 h-16 bg-white rounded-md border border-gray-200 flex items-center justify-center overflow-hidden">
                      <img
                        src={opt.imageDataUrl || "/placeholder.svg?height=64&width=64"}
                        alt={`옵션 ${index + 1}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-gray-900 truncate">선택지 {index + 1}</span>
                        <button
